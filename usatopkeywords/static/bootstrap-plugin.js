...
$.alert_pro = function(subject, ok_callback) {
	var options = {size: "md"};
	var s = '\
	<div class="modal fade" tabindex="-1" role="dialog">\
		<div class="modal-dialog modal-'+options.size+'">\
	...
	var jmodal = $(s).appendTo('body');
	jmodal.modal('show');
	jmodal.find('.modal-footer').find('.btn-secondary').on('click', function() {
		jmodal.modal('hide');
		ok_callback();
	});

	jmodal.find('.modal-header').find('.close').on('click', function() {
		jmodal.modal('hide');
		ok_callback();
	});
